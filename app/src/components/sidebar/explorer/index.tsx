...
export const Explorer = () => {
	const { currentProject, openFile, activeFile, loadFileTree } =
		useAppStore();
	...
	const handleOpenInTerminal = async () => {
		...
		try {
			await invoke('open_in_terminal', { dirPath: currentProject.path });
		} catch (err) {
			console.error(err);
		}
	};
	...
	const handleOpenFolder = async () => {
		if (!currentProject) return;
		try {
			await invoke('open_in_finder', { path: currentProject.path });
		} catch (err) {
			console.error(err);
		}
	};
	...
	const handleRootInputSubmit = async () => {
		...
		if (!name) {
			setInput((s) => ({ ...s, error: 'Name is required' }));
			return;
		}
		if (/[\\:*?"<>|]/.test(name)) {
			setInput((s) => ({ ...s, error: 'Invalid characters' }));
			return;
		}
		try {
			if (input.type === 'file') {
				...
				<div className="h-9.5 flex items-center justify-between px-3 border-b border-white/4">
					...
					<div className="flex items-center gap-0.5">
						...
						<DropdownMenu>
							...
							<DropdownMenuContent
								align="end"
								className="w-48 bg-[#1e1e24] border-white/10 text-neutral-300 shadow-xl shadow-black/40"
							>
								<DropdownMenuItem
