		...
		return (
			<div className="text-[11px] text-neutral-400 leading-relaxed">
				<ReactMarkdown
					components={{
						p: ({ children }) => (
							<p className="text-[11px] text-neutral-400 leading-relaxed mb-1 last:mb-0">
								{children}
							</p>
						),
						strong: ({ children }) => (
						...
						h3: ({ children }) => (
							<p className="text-[11px] text-neutral-300 font-semibold mt-2 mb-0.5">
								{children}
							</p>
						),
						h2: ({ children }) => (
						...
						code: ({ children }) => (
							<code className="text-[10px] font-mono text-neutral-500 bg-white/4 px-1 rounded">
								{children}
							</code>
						),
					}}
		...
		return (
			<div className="space-y-1.5">
				<div className="flex items-start gap-1.5">
					...
					/>
					<p className="text-[11px] text-red-400/80 leading-relaxed wrap-break-words">
						{log.message}
					</p>
				</div>
				{!agentRunning && (
...
export const LogGroupBlock = ({
	...
	const fileWrites = isScaffold
		? sorted.filter((l) => l.type === 'file_write')
		: [];
	const rest = isScaffold
		? sorted.filter((l) => l.type !== 'file_write')
		: sorted;
	...
	return (
		...
		>
			...
			<div className="flex flex-col items-center shrink-0 w-5">
				<div
					className={`w-5 h-5 rounded flex items-center justify-center ${s.iconBg}`}
				>
			...
			<div className="flex-1 min-w-0 pb-5">
				<span
					className={`text-[9px] uppercase tracking-[0.15em] font-semibold ${s.badge} block mb-2`}
				>
					{s.label}
				</span>
